...
function LoginPageContent() {
  ...
  const requestReset = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (!response.ok) throw new Error(data.error || "So‘rovni yuborib bo‘lmadi.");
      setResetMessage(data.message || "Agar bu email ro‘yxatdan o‘tgan bo‘lsa, parolni tiklash havolasi yuborildi.");
      if (data.resetUrl) {
        setDevResetUrl(data.resetUrl);
      }
    } catch (err: unknown) {
      setResetMessage(err instanceof Error ? err.message : "So‘rovni yuborib bo‘lmadi. Keyinroq qayta urinib ko‘ring.");
    }
  ...
  return (
    <AuthLayout title="Qaytganingizdan xursandmiz" description="O‘qishni davom ettirish uchun hisobingizga kiring.">
      ...
      >
        ...
        {googleLoading ? "Yo‘naltirilmoqda..." : "Google bilan davom etish"}
      </button>
        ...
        <div className="fixed inset-0 z-50 grid place-items-center bg-ink/35 p-4">
          <div role="dialog" aria-modal="true" className="w-full max-w-sm rounded-3xl bg-white p-6 shadow-2xl">
            <div className="flex items-start justify-between">
              <div>
                <h2 className="text-xl font-black text-ink">Parolni tiklash</h2>
                <p className="mt-1 text-sm font-medium text-slate-600">Emailingizga tiklash havolasini yuboramiz.</p>
              </div>
              <button onClick={() => setResetOpen(false)} className="focus-ring rounded-lg p-1 text-slate-500">
                <X className="h-5 w-5" />
              </button>
            </div>
            <form onSubmit={requestReset} className="mt-5 space-y-3">
              <label className="block text-sm font-bold">
                Email
                <input required type="email" value={email} onChange={(e) => setEmail(e.target.value)} className="focus-ring mt-1.5 w-full rounded-xl border px-3 py-3 font-medium" placeholder="siz@email.com" />
              </label>
              {resetMessage && (
                <div className="rounded-xl bg-mint p-3 text-xs font-semibold text-forest space-y-2">
                  <p className="break-all">{resetMessage}</p>
                  {devResetUrl && (
                    <div className="pt-2 border-t border-forest/20">
                      <p className="text-[11px] opacity-80 mb-1">Dev rejimida test havolasi:</p>
                      <Link
                        href={devResetUrl}
                        onClick={() => setResetOpen(false)}
                        className="inline-block font-bold underline break-all hover:text-ink transition-colors"
                      >
                        Parolni tiklash sahifasiga o‘tish &rarr;
                      </Link>
                    </div>
                  )}
                </div>
              )}
              <button disabled={resetLoading} className="focus-ring flex w-full items-center justify-center gap-2 rounded-xl bg-forest px-4 py-3 text-sm font-bold text-white disabled:opacity-60">
                {resetLoading && <Loader2 className="h-4 w-4 animate-spin" />} <Mail className="h-4 w-4" /> Havolani yuborish
              </button>
            </form>
          </div>
        </div>
